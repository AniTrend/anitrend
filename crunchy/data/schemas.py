...
class ResourceSchema(CommonSchema):
    ...
    __links__: String = fields.Mapping(
            allow_none=True,
            unknown=EXCLUDE
    )
    __actions__: String = fields.Mapping(
        allow_none=True,
        unknown=EXCLUDE
    )
    ...
    def __modify_image_body(
        ...
        if "images" in data:
            ...
            if "poster_tall" in images:
                poster_tall = stream(images["poster_tall"]).flatMap().toList()
                data["images"]["poster_tall"] = poster_tall
            if "poster_wide" in images:
                poster_wide = stream(images["poster_wide"]).flatMap().toList()
                data["images"]["poster_wide"] = poster_wide
            if "thumbnail" in images:
                thumbnail = stream(images["thumbnail"]).flatMap().toList()
